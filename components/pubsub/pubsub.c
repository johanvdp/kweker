...
{
    ESP_LOGV(tag, "pubsub_topic_subscribers_t topic:%s", topic_name);
    ...
    LIST_FOREACH(candidate, &pubsub_topic_subscribers, pointers)
    {
        if (strcmp(candidate->topic, topic_name) == 0) {
            // found
            ESP_LOGV(tag, "pubsub_topic_subscribers_t found:%p", candidate);
            return candidate;
        }
    }
    ESP_LOGV(tag, "pubsub_topic_subscribers_t found:none");
    return NULL;
}
...
static pubsub_topic_subscribers_t* pubsub_add_topic_subscribers(
        const char *topic_name)
{
    ESP_LOGV(tag, "pubsub_add_topic_subscribers topic:%s", topic_name);
    ...
    ESP_LOGV(tag, "pubsub_add_topic_subscribers subscribers:%p",
            topic_subscribers);
    ...
    if (topic_subscribers != NULL) {
        ...
        LIST_FOREACH(candidate, &(topic_subscribers->subscribers), pointers)
        {
            if (candidate->queue == subscriber_queue) {
                ESP_LOGD(tag, "pubsub_remove_subscription topic:%s, queue:%p found", topic_name,
                        subscriber_queue);
...
void pubsub_unregister_topic(const char *topic_name)
{
    ...
    if (topic_subscribers != NULL) {
        ...
        LIST_FOREACH_SAFE(subscriber, &(topic_subscribers->subscribers), pointers, subscriber_safe)
        {
...
uint16_t pubsub_topic_count()
{
    uint16_t count = 0;
    pubsub_topic_subscribers_t *topic_subscribers;
    LIST_FOREACH(topic_subscribers, &pubsub_topic_subscribers, pointers)
    {
        ESP_LOGD(tag, "pubsub_topic_count topic:%s", topic_subscribers->topic);
        count++;
    }
